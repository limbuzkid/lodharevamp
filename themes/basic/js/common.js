...
(function($){
    $(document).ready(function() {
        ...
        $(window).on('load', function() {
            ...
            if ($('#map').length && (window.location.pathname.split("/").pop() == 'index.html' || window.location.pathname.split("/").pop() == 'nri-overview.html') ) { initMap(undefined, 1); }
        ...
        if ($('.innerMainBanner').length) {
            function formTop() {
                ...
                $(' body > .getInTouchWrap > .getInTouchLightBox').css({ 'top': banner_height - form_height });
            }
            formTop();
            $(window).resize(function() {
                formTop();
        ...
        var form_offset = $('body > .getInTouchWrap .getInTouchLightBox').length ? $('body > .getInTouchWrap .getInTouchLightBox').offset().top : 0;
        var form_offsetLeft = $('body > .getInTouchWrap .getInTouchLightBox').length ? $('body > .getInTouchWrap .getInTouchLightBox').offset().left : 0;
        ...
        // ------------locate-us Page Ends------------
        // ------------homepage let us work section Starts------------
        var imgSlider, contSlider, tabsSlider;
        $(document).on('click', '.letsWorkBetter .projectsTab li a', function() {
        ...
        // ------------homepage let us work section Starts------------
        //$('.owl-nav , .owl-dots').wrapAll('<div class="child"></div>');

        // var hightestbox = 0;
        // $('.box').each(function(){
        //     if($(this).outerHeight() > hightestbox){
        //         hightestbox = $(this).outerHeight();
        //     }
        // });
        // $('.box').css('height', hightestbox);
